...
When given a problem, most students usually jump straight into the coding part. This approach seems logical at first glance. When you are in the interview setting time is limited and you want to solve the problem as fast as possible. Writing algorithm seems like unnecessary usage of time. However, in this guide, we want to show you that following PEDAC process will, in the long run, actually save you time and enable you to solve **complex** problems in much efficient way.
...
Let's consider another problem. Try to go through the "understand the problem" part of this problem yourself and  write input, output and rules for it. We will provide you with solution below. Later, we will tackle data structure and algorithm part.
...
The algorithm writen above is what we usually see students write for similar problems. It looks complete, but let's think for a second what is the hardest part of the given problem? Is it looping through an array and pushing substrings that are palindromes in the result array? Is it maybe determening whether a substring is a palindrom? Or is it something else?
...
The main takeaway here is that you need to write complete algorithm for a problem, including all the hard parts of it (like our `substrings` method from above) because if you can't write in plain English solution to the problem, you will not be able to code it as well. You also do not need to know any "fancy" methods to solve this problems. Yes, knowing `Integer#upto` method would make this problem easier,but even if you do not know it, problem can be solved with simple looping as we have shown you.
